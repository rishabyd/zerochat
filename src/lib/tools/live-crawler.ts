...
export const urlCrawler = tool({
  ...
  inputSchema: z.object({
    urls: z.array(z.url()).min(5).max(50).describe('Array of URL strings to crawl (1-10 URLs)'),
    subpages: z
      .number()
      .min(1)
      .max(5)
  ...
  execute: async ({ urls, subpages = 5, include_links = true }) => {
    try {
      console.log(`🕷️ Crawling ${urls.length} URLs with up to ${subpages} subpages each`);
      ...
      const urlResults = urls.map((url) => {
        const urlPages = result.results.filter((r) => r.url === url || r.url?.startsWith(url));
      ...
      console.log(`✅ Crawled ${result.results.length} total pages from ${urls.length} URLs`);
